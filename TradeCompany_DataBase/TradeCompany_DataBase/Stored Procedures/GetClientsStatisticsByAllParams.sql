	...
	@ToOrdersCount int
	AS
	select O.ClientsID as ID, C.RegistrationDate, C.Name, count(OL.OrderID) as OrdersCount, sum(OL.Price * OL.Amount) as TotalAmount, C.LastOrderDate
...
AND (@ToAmount is null or sum(OL.Price * OL.Amount) <= @ToAmount)
